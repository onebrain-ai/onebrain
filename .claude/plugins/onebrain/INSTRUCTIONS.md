...
- Run session initialization from the vault root:
  - **Codex with a SessionStart-injected token:** run `onebrain session init --json --session-token {injected_session_token}`. Store the returned `session_token` and verify it exactly equals the injected token. If it differs or the flag is unavailable, preserve the injected token and disable checkpoint/wrapup writes for this chat rather than falling back to terminal/process identity.
