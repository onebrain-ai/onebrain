...
- The launchd plist generated by `onebrain schedule register` for a one-shot entry runs the skill once, then calls `launchctl bootout` + `rm -f` to remove itself — the wizard does not need to schedule cleanup; the plist handles it.
