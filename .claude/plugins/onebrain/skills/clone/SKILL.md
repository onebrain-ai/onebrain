...
- **`qmd_collection` in `onebrain.yml` is vault-specific.** When cloning to a new vault, the `qmd_collection` value will point to the old vault's collection. The user must set up the search index for the new vault (via the `onebrain` CLI or the web service) to create a collection for the new vault and update `onebrain.yml` accordingly.
