...
**Scheduler health** — the per-entry checks below run only when `onebrain.yml` has a `schedule:` block; the **Stale plist content-shape** check runs whenever any installed `com.onebrain.*.plist` exists, regardless. All plist paths are macOS/launchd-specific — on non-macOS hosts the glob is empty and these checks no-op.
...
📅 Scheduler                          (only if `schedule:` block present in onebrain.yml AND has findings)
