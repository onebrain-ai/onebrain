...
`fix` is a **top-level array**, a sibling of `checks` — not a field inside each check object. It is present only when `--fix --json` was invoked, and its entries are matched back to checks by the `check` name.
...
When `--fix --json` returned `fix[]`, render each entry's `outcome` under the matching check; the post-fix `checks[]` reflects state after the fix attempts. The CLI emits exactly four outcomes:

| `outcome` | Render | Meaning |
|---|---|---|
| `fixed` | ✓ fixed | Recipe ran cleanly; the issue should be resolved. |
| `partial` | 🟡 partially fixed | Recipe made real progress but part of the issue remains. `message` itemises both halves — show it. |
| `failed` | ✕ failed | Recipe was attempted and did not finish. `message` explains why. |
| `manual` | → manual | No automated recipe exists. `message` is the command for the user to run. **Not a failure** — do not render it as one. |

**`manual` is common, not exceptional.** On a vault with a warm daemon — the normal configuration — the CLI's `lex-index` check reports "could not verify" as a warning with no automated repair, so `manual` appears on nearly every `--fix` run. Treat any outcome string outside this table as unknown: render it verbatim and carry on. Never treat an unrecognised outcome as an error — that would report a healthy vault as broken.
...
**Scheduler health** — cross-platform since CLI v3.4.20 (macOS launchd · Windows Task Scheduler · Linux systemd user timers). **Ask the CLI, not the filesystem**: `onebrain schedule list` reports each entry's real state by querying the OS scheduler (`launchctl print` / `schtasks /Query` / `systemctl --user is-active`), and it owns the label derivation — command-mode labels carry an args/cron/at discriminator since v3.4.16, so re-deriving artifact names in this skill WILL drift. The log-file checks run only when `onebrain.yml` has a `schedule:` block; the per-entry state check runs whenever `schedule list` reports entries; the orphan-artifact check runs whenever installed artifacts exist, regardless of the block.
...
- **Entry state** (replaces the old *Missing plist* drift check): run `onebrain schedule list` and read each entry's status glyph. Per entry: `✗` (nothing installed) → 🟡 `schedule entry {label} not installed — onebrain schedule register to repair`; `⚠` (artifact present but the OS scheduler is not running it) → 🟡 `schedule entry {label} installed but inactive — onebrain schedule register to re-activate`. If the CLI is older than v3.4.20 and `schedule list` fails, fall back to the macOS-only plist-existence check and note the CLI is outdated.
- **Orphan artifacts** (an artifact left behind after the user cleared or renamed its `schedule:` entry). Per host OS, list installed artifacts and flag any that `schedule list` does not account for → 🟡 `stale schedule artifact: {name} — onebrain schedule register --remove` (then re-`register` if entries remain). When unsure whether an artifact maps to a current entry, do **not** flag it (favor false-negative over false-positive).
  - macOS: glob `~/Library/LaunchAgents/com.onebrain.*.plist`
  - Windows: `schtasks /Query /FO CSV | findstr \OneBrain\` (tasks live under the `\OneBrain\` folder; no files on disk by design)
  - Linux: glob `~/.config/systemd/user/onebrain-*.timer` (each entry also has a matching `.service`; a `.timer`/`.service` present without its sibling is itself a 🟡 finding)
- **One-shot reachability**: for each `at:` entry, verify the timestamp hasn't already passed. One-shots self-delete after firing on every platform, so *passed timestamp + artifact still installed* → 🟡 `expired one-shot {label} not cleaned up — onebrain schedule register --remove`.
- **Last-run outcome** ([onebrain#230](https://github.com/onebrain-ai/onebrain/issues/230)): ask the OS scheduler how each installed entry's *most recent* run ended — this is the detection gap behind the 2026 silent-failure incident (a schedule that fired exit 78 for months with no visible signal). Never *run* an entry to find out; only query state:
  - macOS: `launchctl print gui/$(id -u)/com.onebrain.<label>` → parse `last exit code = N`. `N ≠ 0` → 🟡 `last run of {label} failed (exit {N}) — check ~/Library/Logs/onebrain/`. Field absent (never ran) → skip.
  - Linux: `systemctl --user show onebrain-<label>.service --property=ExecMainStatus,ExecMainExitTimestamp` → nonzero `ExecMainStatus` with a real exit timestamp → 🟡 `last run of {label} failed (exit {N}) — journalctl --user -u onebrain-<label>.service -n 20`. Empty timestamp (never ran) → skip.
  - Windows: `schtasks /Query /TN "\OneBrain\<label>" /FO LIST /V` → parse `Last Result`. Nonzero and not `267011` (= task has never run) → 🟡 `last run of {label} failed (result {code}) — see Task Scheduler history`.
  - Get `<label>` per entry from `onebrain schedule list` output — do not re-derive it.
- **Unsupported host**: if `onebrain schedule list` reports the platform has no scheduler backend, emit one explicit line — `⏭ scheduler checks skipped: no scheduler backend on this OS` — instead of silently omitting the section. (All three major desktop platforms have backends since v3.4.20; this line exists so exotic hosts don't read "no findings" as "healthy".)

> The pre-v2.3.3 **stale plist content-shape** check (Bun-era `--headless`/`run-skill`/`.ts` argv shapes) was removed 2026-07-29 per its own sunset note — the Bun CLI was deprecated 2026-06-30 and any plist that old now trips the orphan/state checks above instead.
...
📅 Scheduler                          (only when there are scheduler findings — orphan-artifact findings can occur even with no `schedule:` block; the ⏭ unsupported-host line always shows when it applies)
  <errors, consecutive failures, entry state (✗ missing / ⚠ inactive), orphan artifacts, expired one-shots>
...
1. **CLI fixes** — already executed by `onebrain doctor --fix --json` in Step 2a. The top-level JSON `fix[]` array reports outcomes (`fixed`, `partial`, `failed`, `manual` — see the outcome table in Step 2a). Render under each affected check.
