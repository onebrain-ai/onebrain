...
## Step N: Auto-Finalize Active Pause Thread (if any)

Before writing the daily session log:

1. Read `[logs_folder]/pause/_active.md`. If absent or empty → skip this step entirely.
2. If a slug is present, apply the three skip conditions from `skills/pause/SKILL.md` → Auto-Finalize section:
...
3. If not skipped: invoke `/pause` auto-finalize path (Steps 2–5 of `/pause`, with `trigger: auto-finalize` in frontmatter and "Auto-finalized at session end. " prefix in `## Where I Stopped`).

This preserves the active pause thread's continuity when a session ends without an explicit `/pause`. No user-visible output (AUTO-SUMMARY is silent).
