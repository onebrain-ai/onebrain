...
- **The marker now carries a boundary** (`<!-- recovery-of: {token}:{date}:through-NN -->`) and the group is partitioned: `preserved` (`NN` <= boundary, deletable) vs `unpreserved` (`NN` > boundary, never deleted on the marker's strength — recovered into a new log instead).
- **The boundary is the checkpoint number, deliberately not an mtime** — `/recap` edits session logs afterwards (it adds `recapped:`) and sync rewrites mtimes; either pushes the log forward and would reintroduce the bug, whereas `NN` lives in the filename and is never rewritten.
- **Legacy markers state no boundary**, so the whole group is re-recovered: one duplicated log, once, then step (g) deletes the group — duplication is an inconvenience, deletion is unrecoverable.
...
- Step (e) writes the boundary, step (f) verifies it re-reads at the right value, and a repeat marker for one `token:date` is documented as expected (it raises the boundary) rather than something to deduplicate.
