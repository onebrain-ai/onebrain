...
- **Step 7 no longer reports deleted files as "left on disk"** — `already_recovered` records whose file step (g) went on to delete are dropped, and a path that collected both `already_recovered` and `delete_failed` is rendered once.
