...
- **If nothing could be read — or hashed — no log is written at all.** Without that cut-off an unreadable checkpoint made every /wrapup emit a fresh contentless session log forever, since the group can never empty when deletion is gated on `consumed:`. The same applies to a checkpoint whose hash cannot be computed: it is excluded from the recovery set rather than driving an abort-and-retry loop, which matters most when the hash tool is missing or blocked outright and every group in the vault would otherwise duplicate on every run.
- **Whichever step exits early owns the sweep of `preserved` — but only when the exit is a fixed point.** Step (a)'s `unpreserved`-is-empty branch and step (b)'s cut-off will be reached identically by every future run, so they sweep; step (f)'s abort and step (g)'s concurrency abort are retried whole and must not, the latter firing exactly when the guard has proved the owning session is alive. Exactly one site ever sweeps a given group.
