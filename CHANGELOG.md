...
- All three harnesses' hook commands now wrap `onebrain hook` in a shell-level `|| echo` fail-open fallback, so rolling the CLI back below 3.4.25 degrades to an inert warning (or empty object) instead of a blocking exit code — Claude's `check-cli-version.sh` gate remains the user-facing messaging layer.
...
## v3.4.1 — 2026-07-21 — /wrapup: checkpoint deletion is authorised by content, not by a marker

- **Fixed: orphan recovery deleted checkpoints it had never read.** An anchored `<!-- recovery-of: {token}:{date} -->` marker proves only that *a* recovery happened for that token and date — a session that keeps running writes more checkpoints under both, and the check false-positived on every one. Observed live: a log written 16:38 carried the marker while the owning session wrote checkpoints 04-07 at 16:40-22:36, none of whose content was in it.
- **Deletion is now content-addressed.** A recovered log carries a `consumed:` frontmatter list — one `{file, sha256}` entry per checkpoint whose content it actually holds, written from what was READ — and a checkpoint is deleted only when its filename *and* a re-hash both match. The marker's only remaining job is locating candidate logs.
- **Why not a checkpoint number or an mtime.** `NN` is derived by scanning the checkpoint directory, so /wrapup's own delete restarts it at `01` the same day and a stored boundary would swallow the fresh files (observed: token `4e57aeee` reached `09`, the group was deleted, the next hook reported `01 since start`). A reused filename collides exactly; `/recap` and sync both push mtimes forward. Only content is stable.
- **Every checkpoint lands in exactly one bucket** — `preserved`, `excluded` (hash uncomputable), or `unpreserved` — assigned in one place, with the Fail-safe as the sole stated exception. `excluded` files never enter a recovery, because a file that can never gain a `consumed:` entry can never be deleted and would otherwise be re-recovered into a fresh log every run, forever.
- **Scans are keyed on the token, never on a date.** Step 1 no longer globs today+yesterday (which stranded a three-day session's day-3 checkpoints permanently), markers are found by content rather than by log filename (a cross-midnight group files them under an earlier date's name and `YYYY/MM`), and step (g)'s concurrency re-glob dropped its date component, which had blinded it to the likeliest proof a session is alive.
- **Exactly one site sweeps `preserved`, and the three are enumerated, not derived** — step (g), step (a)'s `unpreserved`-is-empty branch, step (b)'s read cut-off. The two abort paths hand the group over intact and must not sweep; step (g)'s concurrency abort fires precisely when the guard has proved the owning session alive. Early exits skip through step (h), so no report claims a session that was never written.
- **Reporting matches disk.** Deleted files are no longer listed as "left on disk", records collapse to one per path by precedence, and an aborted recovery either repairs its log in place or removes it and says so — never leaves, or points at, a claim it just disproved.
- **Two false safety claims corrected.** The Active-Session Guard's "non-destructive" note held only in one direction; and re-hashing catches copied or stale entries but cannot catch an entry for a present, correctly-hashing file whose content never reached the log body — that gap is now named, with the write-from-what-you-read rule identified as the only thing standing in it.
