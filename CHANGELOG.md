...
- **If nothing could be read, no log is written at all.** Without that cut-off an unreadable checkpoint made every /wrapup emit a fresh contentless session log forever, since the group can never empty when deletion is gated on `consumed:`.
...
- **Step 7 no longer reports deleted files as "left on disk"** — `already_recovered` records are dropped for any file this run deleted at *either* deletion site (step (a)'s `unpreserved`-is-empty branch as well as step (g)), and records collapse to one per path by explicit precedence so a single file is never listed or counted twice.
