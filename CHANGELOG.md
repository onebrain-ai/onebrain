...
- **One partition decides everything, in one place.** A checkpoint lands in exactly one of `preserved`, `excluded` (hash uncomputable), or `unpreserved`, and no rule elsewhere may re-decide it. The rules for an un-hashable file had been spread across the partition, the Fail-safe, and a trailing note with no stated precedence, so the same trigger had three answers and the unbounded-duplicate-log loop survived under two of them.
