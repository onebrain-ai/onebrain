...
- **Exactly one site sweeps `preserved`, decided by whether step (g) runs in this run** — not by predicting whether the condition will recur. Step (a)'s `unpreserved`-is-empty branch and step (b)'s cut-off end the group before (g), so they sweep; step (f)'s abort and step (g)'s concurrency abort hand the group to the next run intact and must not, the latter firing exactly when the guard has proved the owning session is alive.
