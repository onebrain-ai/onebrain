...
- **Fixed: the already-recovered short-circuit deleted checkpoints it had never read.** An anchored `<!-- recovery-of: {token}:{date} -->` marker proves only that *a* recovery happened for that token and date; a session that keeps running writes more checkpoints under the same token and date, and the check false-positived on every one. Observed live: a log written 16:38 carried the marker while the owning session wrote checkpoints 04-07 at 16:40-22:36, none of whose content was in it — the rule as written would have deleted an entire release epic's history.
- **Deletion is now authorised by content, not by a number.** A recovered log carries a `consumed:` frontmatter list — one `{file, sha256}` entry per checkpoint whose content it actually holds — and /wrapup deletes a checkpoint only when both its filename and a re-hash of its content match an entry. The marker reverts to `<!-- recovery-of: {token}:{date} -->` and does nothing but locate candidate logs.
- **Why not a checkpoint-number boundary** (the approach this release started with): `NN` is not stable, and what destabilises it is /wrapup's own delete. The CLI derives the next number by scanning the checkpoint directory (`max_checkpoint_nn`, "Derive NN from disk"), so once a group is deleted a session resuming **the same day** restarts at `01` — a stored boundary of `through-10` would then swallow the fresh `01`-`03` and delete them unread. Observed live: token `4e57aeee` reached checkpoint `09`, /wrapup deleted the group, and the next Stop hook reported `01 since start`. A bare filename fails identically, because the reused name collides exactly.
- **Why not an mtime either** — `/recap` edits session logs afterwards (it adds `recapped:`) and sync rewrites mtimes; either pushes the log forward and reintroduces the bug. A content hash moves only when the content moves.
- **Markers are located by content, not by filename** — a recovered log is named for the earliest date it covered, so a group spanning midnight (or a month boundary) files its later dates' markers under an earlier date's name and `YYYY/MM` directory, where a filename glob can never find them.
- **`consumed:` is written from what was READ, never from what was found** — a step (b) read failure must leave the entry out, and step (f) re-hashes every entry before it is allowed to authorise anything. An abort now also neutralises the log it just proved wrong, instead of leaving a false claim on disk for later runs to trust.
- **Logs with no `consumed:` list preserve nothing** — including bare-marker logs and any written during this release's interim `:through-NN` form. Their checkpoints are re-recovered: one duplicated log, once, then step (g) deletes the group. Duplication is an inconvenience, deletion is unrecoverable.
- **Step (g)'s concurrency re-glob is token-only, with no date component** — it matched a single date while groups are keyed by token alone, so it was blind to the likeliest proof of a live session: waking up and writing a checkpoint under tomorrow's date. The mtime arm cannot see that either, since an existing file's mtime does not change when a new file appears.
- **`marker_write_failed` remedy 2 no longer tells the user to delete the checkpoints** — it said to delete both the recovered log and its checkpoints and let the next run re-recover, but those are the only two copies; it now says to delete the log only.
- **Partial recovery no longer trips step (g)'s concurrency guard** — the narrowed set is a new `recover_files`, leaving `group_files` intact as the re-glob baseline.
- **Step 1 no longer strands checkpoints older than yesterday** — it globbed today+yesterday while Step 1b excludes the current token by design, so a three-day session's day-3 checkpoints were invisible to both paths forever; it now matches on the token with no date filter.
- **Corrected a false safety claim** — the Active-Session Guard's "non-destructive" note holds only for judging a *dead* group *active*; the reverse reads those checkpoints, relocates them into another session's log, and deletes them.
- Step (e) writes the `consumed:` list, step (f) verifies it by re-hashing, and a repeat marker for one `token:date` is documented as expected (the two logs hold disjoint entries and step (a) reads the union) rather than something to deduplicate.
- **The delete set is what `consumed:` proves preserved, never the whole group.** Step (e) may legitimately omit a checkpoint whose read failed (an unsynced cloud placeholder is the ordinary case), so deleting the full group would have destroyed an unread file with its content in no log — reached through the one direction step (f) does not check, since it verifies entries against disk and never disk against entries. Files left behind are recorded `unread` so the omission reaches the user instead of looking like a clean sweep.
- **One partition decides everything, with one stated exception.** A checkpoint lands in exactly one of `preserved`, `excluded` (hash uncomputable), or `unpreserved`; only the Fail-safe may re-decide it, and only by re-bucketing the whole group when the evidence the partition relied on proves untrustworthy — in which case any `preserved` already assigned from that same scan is discarded rather than allowed to authorise a delete. The rules for an un-hashable file had been spread across the partition, the Fail-safe, and a trailing note with no stated precedence, so the same trigger had three answers and the unbounded-duplicate-log loop survived under two of them.
- **Nothing that can never be credited enters a recovery.** An un-hashable checkpoint can never gain a `consumed:` entry, so recovering it writes a log it can never be credited to and the next run repeats that verbatim. It is now excluded, reported, and kept. This matters most when the hash tool is missing or blocked outright, which fails every group in the vault at once.
- **Exactly one site sweeps `preserved`, and the three sweeping sites are enumerated rather than derived** — step (g), step (a)'s `unpreserved`-is-empty branch, and step (b)'s cut-off. Step (f)'s abort and step (g)'s concurrency abort hand the group to the next run intact and must not sweep, the latter firing exactly when the guard has proved the owning session is alive. Successive drafts each tried to derive the list from a one-line test, and each test misclassified one of the four exits; the list is now the spec and deriving it is explicitly forbidden.
- **Early exits skip through step (h), not just step (g)** — stopping at (g) left the report claiming an auto-recovered session for a log that was never written. The read cut-off also triggers on an empty recovery set, not only on reads that failed, so a vacuous reading cannot fall through and emit a contentless log.
- **The `sha256` definition is normative in both the skill and `session-formats.md`** — raw file bytes, first 16 lowercase hex characters. The earlier wording ("from the file you read") was readable as the *extracted content*, which two executors would compute differently, and a form stripping the frontmatter date/token/`NN` could make two different files hash equal under a reused filename.
- **A step (f) abort no longer leaves, or misreports, a log it proved wrong** — bad entries are stripped in place (keeping the path reportable and repairable); only an unwritable log is removed, and that case reports `log_removed` rather than claiming a saved log and offering remedies that open a file that is gone.
- **Step 7 no longer reports deleted files as "left on disk"** — `already_recovered` records are dropped for any file this run deleted at *any* deletion site, tested against the on-disk state at render time rather than against a list of steps, and records collapse to one per path by explicit precedence so a single file is never listed or counted twice.
- **Narrowed a false safety claim** — re-hashing catches copied and stale entries, but cannot catch an entry for a file that is present and hashes correctly yet whose content never reached the log body. That gap is now named, with step (e)'s write-from-what-you-read rule identified as the only thing standing in it.
...
- **Kept as `qmd` for now — matches the current CLI** (all versions incl. main still emit these; renaming the plugin first would break parsing/registration): the session-init JSON field `qmd_unembedded` (CLI v3.4 contract, tests lock it — the CLI now *reads* the value from `search.collection` but still names the field `qmd_unembedded`), and the "PostToolUse qmd hook" name + its top-level-`qmd_collection` trigger (`register_hooks/mod.rs` gates on `cfg.qmd_collection`; the hook command itself already migrates to `onebrain search reindex`). These rename to `search_*` in a later **CLI-first** follow-up (CLI renames the field/hook — emitting both names for a transition version — then a plugin PR reads the new names with a version-gated fallback), still under the untagged v3.4.5.
...
- **perf(doctor): delegate the 8 schema-validation checks to `onebrain doctor --json`** — skill SKILL.md trimmed 290 → 205 lines (-29%). The skill now runs a single subprocess call for `vault.yml`, `vault.yml-keys`, `folders`, `plugin-files`, `settings-hooks`, `orphan-checkpoints`, `qmd-embeddings`, `claude-settings` (Rust-native, structured JSON). Skill-only checks (broken wikilinks, orphan notes, MEMORY.md size, inbox backlog, scheduler health, pause-thread state, memory health) remain in the skill — those are vault-content / state-machine checks the CLI doesn't cover.
- **fix(doctor): `--fix` flow delegates to `onebrain doctor --fix --json`** for CLI-side fix recipes (settings-hooks, plugin-files, vault.yml-keys, claude-settings, qmd). Skill `--fix` passes (wikilink fuzzy-match, MEMORY.md structure, confidence-score updates) remain skill-side via `references/autofix-procedures.md`.
- **chore: remove dead GitHub workflows** — `ci.yml` (`bun install` + `tsc --noEmit` + `biome check` + `bun test`) and `release.yml` (binary build via `bun build --compile` from `src/index.ts` + npm publish) targeted the v2.x TypeScript CLI code that was removed in v3.0.2. Plugin repo has no test/build surface anymore; CLI workflows live at [`onebrain-ai/onebrain-cli/.github/workflows/`](https://github.com/onebrain-ai/onebrain-cli/tree/main/.github/workflows). Resolves "next PR would fail CI on missing tsconfig/package.json/src" footgun.
- **`--vault` / `--config` flag semantics tightened**: `--vault` now skips the CLI subprocess call (vault content checks only); `--config` skips skill-side content checks (CLI + memory left out). Empty report sections are omitted entirely. Other skills unchanged. CLI v3.0.0 floor pin retained (`requires.cli >=3.0.0`).
